...
class lastfmApiAuth extends lastfmApi {
	 ...
	 * @access public
	 */
	public function __construct($method, $vars = NULL) 
	{		
		parent::__construct();
		$this->_obj =& get_instance();
		$this->_obj->load->config('lastfmapi');
		
		$this->apiKey = $this->_obj->config->item('apiKey');
		$this->secret = $this->_obj->config->item('secret');
		$this->username = $this->_obj->config->item('username');
		$this->sessionKey = $this->_obj->config->item('sessionKey');
		$this->subscriber = $this->_obj->config->item('subscriber');
